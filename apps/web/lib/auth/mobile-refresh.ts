...
// PEPPER: token hash'lerine eklenen gizli sabit — DB ele geçerse bile token'ları korutur.
// Üretmek için: openssl rand -hex 32
const REFRESH_TOKEN_PEPPER = process.env.MOBILE_REFRESH_TOKEN_PEPPER || '';

if (!REFRESH_TOKEN_PEPPER && process.env.NODE_ENV === 'production') {
    console.error('[MobileRefreshService] MOBILE_REFRESH_TOKEN_PEPPER is not set in production. Refresh tokens are less secure.');
}
