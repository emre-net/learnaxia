...
export class OpenAIAIProvider implements AIProvider {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                if (attempt === MAX_RETRIES) {
                    // Max deneme sayısına ulaşıldı — DB'ye loglandı
                    break;
                }
                ...
                if (evalParsed.isValid) {
                    // DB'ye loglandı (SystemLog)
                    break;
                } else {
                    // Checker feedback DB'ye kaydedildi, bir sonraki attempt'e geçiliyor
    ...
    async analyzeImage(imageBuffer: Buffer, mimeType: string, language: string = 'tr'): Promise<VisionResult> {
        ...
        try {
            const response = await this.client.chat.completions.create({
                model: "meta-llama/llama-4-scout-17b-16e-instruct", // Groq'un vision-capable modeli
                messages: [
                    { role: "system", content: systemPrompt },
                    {
                        role: "user",
    ...
    async generateJourneySlide(topic: string, parentTopic: string, depth: string, language: string = "tr"): Promise<z.infer<typeof SlideGenerationSchema>> {
        ...
        const baseSystemPrompt = `
            ...
            3. Provide a "peekingQuestion" object with the following fields:
               - "question": (string)
