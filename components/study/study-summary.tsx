...
export function StudySummary() {
    ...
    const xpEarned = correctCount * 10 + 50; // Simple XP logic
        ...
        <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950">
            <Card className="w-full max-w-md p-8 text-center flex flex-col items-center gap-6 shadow-xl border-2">
                <div className="relative">
                    ...
                    <Trophy className="h-24 w-24 text-yellow-500 relative z-10 animate-bounce" />
                </div>

                <div className="space-y-2">
                    <h1 className="text-3xl font-bold">Session Complete!</h1>
                    <p className="text-muted-foreground">Great job keeping up with your studies.</p>
                </div>

                <div className="grid grid-cols-2 gap-4 w-full">
                    <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800">
                        <div className="text-3xl font-bold text-green-600 dark:text-green-400">{accuracy}%</div>
                        <div className="text-xs text-green-600/80 uppercase font-semibold">Accuracy</div>
                    </div>
                    <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
                        <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">+{xpEarned}</div>
                        <div className="text-xs text-blue-600/80 uppercase font-semibold">XP Earned</div>
                    </div>
                </div>

                <div className="flex gap-8 text-sm text-muted-foreground">
                    <div className="flex items-center gap-2">
                        <CheckCircle2 className="h-4 w-4 text-green-500" /> {correctCount} Correct
                    </div>
                    <div className="flex items-center gap-2">
                        <RotateCcw className="h-4 w-4 text-red-500" /> {wrongCount} Review
                    </div>
                </div>
